...
def update_hyperparameters(self, mufbvar_data, pbounds, init_points, n_iter, nsim, save = False, name = "hyp.txt"):
    ...
    def estim(hyp_list, nsim):
        ...
        nburn = round((self.nburn_perc)*nsim)
        self.nburn = nburn
        ...
        Sigmap_list.append(np.zeros((round((nsim)/self.thining),nv_list[0],nv_list[0])))
        Phip_list.append(np.zeros((round((nsim)/self.thining),int(nv_list[0])*int(p_list[0])+1,int(nv_list[0]))))
        Cons_list.append(np.zeros((round((nsim)/self.thining),nv_list[0])))
        lstate_list.append(np.zeros((round((nsim)/self.thining),Nq_list[0],int(Tnobs_list[0]))))
        YYactsim_list.append(np.zeros((round((nsim)/self.thining),freq_ratio_list[0]+1,nv_list[0])))
        XXactsim_list.append(np.zeros((round((nsim)/self.thining),int(freq_ratio_list[0])+1,int(nv_list[0])*int(p_list[0])+1)))
            ...
            for m in range(len(YMh_list)):
                ...
                if m < (len(YMh_list)-1):
                    if j == 0:
                        ...
                        # Parameter estimation
                        # Matrices for collecting draws from Posterior Density
                        Sigmap_list.append(np.zeros((round((nsim)/self.thining),nv_list[m+1],nv_list[m+1])))
                        Phip_list.append(np.zeros((round((nsim)/self.thining),int(nv_list[m+1])*int(p_list[m+1])+1,int(nv_list[m+1]))))
                        Cons_list.append(np.zeros((round((nsim)/self.thining),nv_list[m+1])))
                        lstate_list.append(np.zeros((round((nsim)/self.thining),Nq_list[m+1],int(Tnobs_list[m+1]))))
                        YYactsim_list.append(np.zeros((round((nsim)/self.thining),freq_ratio_list[m+1]+1,nv_list[m+1])))
                        XXactsim_list.append(np.zeros((round((nsim)/self.thining),int(freq_ratio_list[m+1])+1,int(nv_list[m+1])*int(p_list[m+1])+1)))
